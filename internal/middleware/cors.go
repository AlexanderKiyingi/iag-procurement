...
// Env aliases: CORS_ALLOW_ORIGIN, CORS_ORIGIN, ALLOWED_ORIGINS.
...
// HasWildcardCORS reports whether the configured allowlist includes "*".
func HasWildcardCORS(allowed string) bool {
	if allowed == "*" {
		return true
	}
	for _, o := range splitAllowedOrigins(allowed) {
		if o == "*" {
			return true
		}
	}
