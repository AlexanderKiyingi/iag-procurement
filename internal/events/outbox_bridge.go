...
func BuildInvoiceReceived(documentRef, vendorRef, amount, currency string, dueDate *time.Time) (key string, payload []byte, err error) {
	now := time.Now().UTC().Format(time.RFC3339Nano)
	data := map[string]any{
...
func BuildGrnPosted(grnID, poID, vendorID, receivedBy string, lines []GrnPostedLine) (key string, payload []byte, err error) {
	now := time.Now().UTC().Format(time.RFC3339Nano)
	data := map[string]any{
		"grn_id":      grnID,
		"po_id":       poID,
